...
- `wrangler deploy` (2026-07-10, Etapa 38): Worker `armagedon-api` publicado com version ID `10247d19-ed9b-4e21-9e3a-b3570a5b7160` — desenhos fim-a-fim: `mesa:drawings:update` em `RELAY_TYPES` do DO (jogador tambem desenha; relay valida array, remove tracos `dm` e limita a 300) e novo campo `drawings` na cena oficial (`src/mesa.js`: `normalizeSceneDrawing` + filtro `dm` no GET para nao-mestres). Dry-run limpo antes; health 200 pos-deploy.
