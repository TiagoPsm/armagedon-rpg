...
- **Ferramenta de texto retirada (Etapa 126)**: `normalizeSceneDrawing` (`src/mesa.js`) voltou a aceitar so `pencil|line|rect|circle|cone|arrow` — `text` saiu da whitelist junto com os campos `text`, `size` e `wrap` que as Etapas 123 e 125 tinham adicionado. Rotulo gravado em cena antiga fica no banco mas e descartado no proximo save e o cliente ja nao o desenha. **A retirada so vale em producao depois de `wrangler deploy`**; ate la o Worker publicado continua aceitando `text`
...
- `wrangler deploy` (2026-08-27, Etapas 123-126): Worker `armagedon-api` publicado com version ID `53e1c74a-53b0-488d-bde5-5013a68527f9` — **contrato de desenho atualizado**. `src/mesa.js` `DRAW_TOOLS` passou de `pencil|line|rect|circle` para `pencil|line|rect|circle|cone|arrow`: sem este deploy, cone e seta apareciam na tela de quem desenhou e sumiam no F5 (a whitelist descarta em silencio o que nao conhece). No mesmo passo, `text` NAO entrou — a ferramenta de texto existiu apenas no repositorio, entre as Etapas 123 e 125, e foi retirada na Etapa 126 antes de qualquer deploy; nenhum campo `text`, `size` ou `wrap` chegou a ser aceito em producao. Cone e seta reaproveitam `x1,y1 -> x2,y2` (origem e ponta), entao nao houve campo novo nem mudanca de schema. Dry-run limpo antes; health 200 pos-deploy.
- `wrangler deploy` (2026-08-19, pos-Etapa 96): Worker `armagedon-api` publicado com version ID `81f01c5f-2fde-42da-bf52-0f8d10407c7f` — leva ao ar as rotas de **pastas de cena** (Etapa 96: `/api/mesa/scene-folders`, `/api/mesa/scenes/:id/folder`) e o **`mapUrl`/`tokenCount`** de `listMesaScenes` (Etapa 89), ambos escritos no codigo mas pendentes de deploy ate agora. Fecha as duas pendencias de deploy em `../DEV_STATUS.md`. Sem mudanca de codigo neste passo. Dry-run limpo antes; publicado sem erro.
...
- `wrangler deploy` (2026-08-28, Etapa 131/138): Worker `armagedon-api` publicado com version ID `1439f8fa-02a1-403c-9f6d-7f906785a2aa` — escala da regua por cena: `src/mesa.js` `normalizeSceneGrid` ganhou `metersPerCell` (clamp 0,1–5000, 2 casas; cena antiga sem o campo cai em 1,5) e a grade DESLIGADA com escala propria deixou de virar `null` — a regua vale sem linha desenhada, entao perder a escala seria o mesmo defeito com outra roupa. Sem mudanca no DO (viaja no `mesa:grid:update`, que ja era master-only). **O codigo estava no repositorio desde 2026-08-27 e so subiu agora**: nesse intervalo o backend descartava o campo em silencio, e a escala escolhida pelo mestre voltava aos 1,5 m no F5 — era exatamente o sintoma relatado. Dry-run limpo antes; round-trip conferido pos-deploy (PUT com `metersPerCell: 30` volta 30 no GET).
  - **Incidente no mesmo deploy** — a verificacao pos-deploy gravou na CENA ATIVA por engano: o `POST /api/mesa/scenes` devolveu o id num formato diferente do assumido, a variavel ficou vazia, e `PUT /api/mesa/scene?id=` com id vazio cai na cena ativa (`resolveSceneIdForActor`), nao em erro. A cena "Mapa Mundi" ficou com 0 tokens. Recuperada por `wrangler d1 time-travel restore armagedon --bookmark=00000618-...` — os 6 tokens voltaram com as posicoes conferidas contra a leitura somente-leitura do mesmo dia. **Licao: id vazio na querystring nao e erro, e silenciosamente a cena ativa. Conferir que o id veio preenchido ANTES de montar a URL.**
