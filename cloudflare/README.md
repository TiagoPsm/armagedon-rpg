...
- **PENDENTE DE DEPLOY (2026-07-30, Etapa 65)**: `src/mesa.js` `normalizeSceneToken` passou a clampar `tokenScale` em 0,25–12 (era 0,25–4), espelhando `MESA_TOKEN_SCALE_MAX` de js/mesa-stage.js. **Sem o deploy o Worker corta em 4 ao salvar a cena** e o token volta a travar em 3x3 no proximo carregamento (o realtime e o localStorage ja aceitam o valor maior). Motivo: o teto de 4 era absoluto, mas o tamanho em celulas depende do tamanho da celula — com celula de ~126px, 4x88px nao chega a 4 celulas. `wrangler deploy --dry-run` limpo (191,65 KiB / 37,09 KiB gzip); deploy aguardando o Tiago.
