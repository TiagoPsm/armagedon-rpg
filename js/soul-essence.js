(function () {
  ...
  function aggregateGains(gains) {
    const byAttr = {};
    gains.forEach(gain => {
      byAttr[gain.attr] = byAttr[gain.attr] || {
        attr: gain.attr,
        amount: 0,
        value: gain.value
      };
  ...
  function getSoulState(core, attributes = {}) {
    ...
    if (normalized.rank >= 7) return "Rank máximo";
  ...
  function addExperience(core, amount) {
    ...
    const application = calculateExperienceApplication(before, amount);
    const nextCore = normalizeSoulCore({
      ...before,
      xp: application.xpAfter,
      pendingNightmare: before.rank < 7 && before.xpLimit > 0 && (application.xpAfter >= before.xpLimit || application.overloadedAfter),
      overloaded: application.overloadedAfter,
      overloadXp: application.overloadXpAfter
    }, before.rank);
  ...
  function buildProgressLabel(core) {
    ...
    if (!normalized.xpLimit) return "Rank máximo alcançado";
