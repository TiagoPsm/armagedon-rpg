...
- **a selecao de token e estado de TELA de cada cliente, nao conteudo da mesa (2026-08-16, Etapa 87)**: nenhum caminho de rede marca token na tela de quem recebe. Delta (`mesa:token:upsert`) nunca aplica o `selectedTokenId` do emissor — todo envelope carrega o de quem enviou, e adotar aquilo deixava o mestre marcar token na tela do jogador. Cena vinda em tempo real (`mesa:scene`) tambem nao mexe na selecao local (`applyMesaSceneSnapshot(..., { keepSelection: true })`) — o Worker transmite a cena sem `clientId`, entao o mestre recebe de volta o proprio eco e adotar a selecao dali desfazia o clique no espaco vazio um instante depois. O campo `selectedTokenId` continua no payload da cena (contrato do Worker/D1), mas so serve ao boot do proprio cliente. Vale tambem para o CACHE: cena remota gravada no `localStorage` leva a selecao deste cliente, nunca a do emissor — senao o estado ao vivo fica certo e o F5 ressuscita o token do outro
