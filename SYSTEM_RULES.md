...
- **`data-mesa-player-only` (2026-08-28, Etapa 134)**: o espelho do master-only, para o caso raro em que a tela do JOGADOR mostra algo que a do mestre nao mostra. Tratado no mesmo `applyMesaRolePermissions`. Existe um unico uso hoje: o aviso "Nada para ajustar por enquanto." dentro do painel da engrenagem, que e verdade para o jogador e mentira para o mestre.
...
Exclusivo do mestre na Mesa (marcado com `data-mesa-master-only`): camadas MESTRE e MAPA, botao ESCAL. (escalacao), botao INIC. (ativar/encerrar combate), controles de conducao do combate, o chrome de mapa (nome do arquivo, limpar mapa, gaveta de cenas), os grupos Grade e Nevoa do painel, biblioteca de mapas, "Travar movimento", "Limpar cena", tabs e busca da escalacao, e o inspetor lateral.

**A engrenagem do mapa NAO e mais exclusiva do mestre (2026-08-28, Etapa 134).** Decisao do Tiago: no canto superior direito o jogador ve a engrenagem e SO ela — sem o nome do arquivo do mapa (que e da pasta local do mestre e nao diz nada a ele) e sem "Limpar mapa" (que nunca funcionou para jogador: `clearActiveMap` recusa quem nao e mestre, entao o botao aparecia morto). O botao ABRE de verdade para o jogador; o painel esta vazio de proposito, com o aviso `data-mesa-player-only`, ate o Tiago definir o que vai ali. `toggleMapSettings()` perdeu o `_requireMapMaster` — o que protege continua sendo cada grupo de dentro, que e master-only por conta propria (Grade por `_isGridMaster`, Nevoa pelo modulo de nevoa), mais o Worker e o DO. O overlay `.vtt-overlay-tr` inteiro era master-only, o que tornava impossivel dar a engrenagem ao jogador; o marcador desceu para cada peca.
